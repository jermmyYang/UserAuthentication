...
*
* 2013-07-29 modified by ken. hour -> minute, minute -> second
*/
...
(function ($) {
    ...
    var timePickerMethods = {
        ...
        setTime: function (hours, minutes) {
            if (hours < 0 || 59 < hours || minutes < 0 || 59 < minutes) {
                throw "Invalid range of specified time.";
            }
            this.each(function () {
                var $this = $(this);
                var options = $this.data("options");
                if (hours === 60 && !options.include24) {
                    throw "Cannot specify 60 to minutes.";
                }
                var min = minutes;
    ...
    var timeRangePickerMethods = {
        ...
        setTimeRange: function (startHours, startMinutes, endHours, endMinutes) {
            if (startHours < 0 || 59 < startHours || startMinutes < 0 || 59 < startMinutes) {
                throw "Invalid range of specified start time.";
            }
            if ((endHours || endHours === 0) && (endHours < 0 || 60 < endHours || endMinutes < 0 || 59 < endMinutes)) {
            ...
            this.each(function () {
                ...
                if (endHours || endHours === 0) {
                    if (endHours === 60 && !options.include24) {
                        throw "Cannot specify 60 to end minutes.";
                    }
    ...
    function bindChange(parent, include24) {
        ...
        endHoursSelect(parent).change(function () {
            ...
            var startHours = startHoursSelect(parent).val();
            var startMinutes = startMinutesSelect(parent).val();
            if(parseInt(endHours) < parseInt(startHours)){
                $(this).val(startHours);
            }
        });

        // added by ken 20130729
        endMinutesSelect(parent).change(function () {
            var endMinutes = $(this).val();
            var startMinutes = startMinutesSelect(parent).val();

            var startHours = startHoursSelect(parent).val();
            var endHours = endHoursSelect(parent).val();

            if(parseInt(startHours) == parseInt(endHours) && parseInt(endMinutes) < parseInt(startMinutes)){
                $(this).val(startMinutes);
            }
        });
    }
